# Supplementary diagram: Box diagram of FUCCs 10lncRNA.
# Read data
...
#2. Screen for genes
gene_overlap
gene_overlap %in% rownames(expr)
data <- expr[gene_overlap,]
...
group_BLIS = group[group$group == 'BLIS',]
dat$Group = ifelse(dat$Sample %in% group_BLIS$sample,"BLIS","normal")
library(ggpubr)
...
ggsave('03-doOverlap-DEG-COX/out-plot//10lnc_fuscc_boxpolt.pdf')
##########################################
...
ggsave(gene_split_violin,
       filename = "03-doOverlap-DEG-COX/out-plot/10lncgene_split_violin.pdf",height = 4.5,width = 6)
