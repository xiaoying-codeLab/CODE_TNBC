...
save(es.max,file = 'gsva_hallmark.Rdata')
load(file = 'gsva_hallmark.Rdata') 
es.max[1:4,1:4]
...
es.max[1:4,1:4]
head(ac)
boxplot(es.max['HALLMARK_TGF_BETA_SIGNALING',] ~ ac$group)
df=data.frame(value=es.max['HALLMARK_TGF_BETA_SIGNALING',],
              group= ac$group)
